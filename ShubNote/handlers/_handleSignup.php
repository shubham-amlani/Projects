                ...
                if($password!=$cpassword){
                    ...
                    header("Location: ../signup.php");
                       ...
                       // Store user data into the database
                        $sql = "INSERT INTO `users` (`user_id`, `username`, `user_email`, `user_pass`, `timestamp`, `verification_code`, `is_verified`) VALUES (NULL, ?, ?, ?, current_timestamp(), ?, '0')";
                        $stmt_insert = $conn->prepare($sql);
                        $stmt_insert->bind_param("ssss", $username, $email, $hash, $code);
                        $stmt_insert->execute();
                        // sendCode($email, $code);
